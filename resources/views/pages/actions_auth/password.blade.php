<x-layouts.minimal title="Restablecer Contraseña">
        ...
        <div class="w-full max-w-md animate-slide-up">
            ...
            <div class="rounded-2xl bg-white p-8 shadow-[var(--shadow-elevated)] ring-1 ring-slate-100">
                <div class="text-center">
                    <h1 class="text-2xl font-semibold text-slate-800 tracking-tight">Nueva contraseña</h1>
                    <p class="mt-1.5 text-sm text-slate-500">Elige una contraseña segura para tu cuenta</p>
                </div>

                <form action="{{ route('password.update') }}" method="POST" class="mt-8 space-y-5">
                    @csrf
                    {{-- Email oculto para el controlador --}}
                    <input type="hidden" name="email" value="{{ $email }}">

                    <div>
                        <label for="password" class="mb-2 block text-sm font-medium text-slate-700">Nueva Contraseña</label>
                        <div class="relative">
                            <x-heroicon-o-lock-closed class="absolute left-3 top-1/2 h-[18px] w-[18px] -translate-y-1/2 text-slate-400" />
                            <input id="password" type="password" name="password"
                                ...
                                @error('password') border-red-300 ring-1 ring-red-200 @enderror"
                    ...
                    <div>
                        <label for="password_confirmation" class="mb-2 block text-sm font-medium text-slate-700">Confirmar Contraseña</label>
                        <div class="relative">
                            <x-heroicon-o-lock-closed class="absolute left-3 top-1/2 h-[18px] w-[18px] -translate-y-1/2 text-slate-400" />
                    ...
                    <button type="submit"
                        ...
                        <span class="uppercase tracking-wide">Guardar contraseña</span>
                    </button>
                </form>
            </div>
